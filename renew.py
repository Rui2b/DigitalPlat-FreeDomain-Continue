...
async def solve_captcha(page):
    """尝试识别并填写验证码"""
    logger.info("尝试寻找验证码...")
    captcha_image_selector = "img#captcha-image"  # 假设验证码图片的选择器
    captcha_input_selector = "input#captcha-input"  # 假设验证码输入框的选择器
    captcha_submit_selector = "button#captcha-submit"  # 假设验证码提交按钮的选择器

    try:
        captcha_image_element = await page.wait_for_selector(captcha_image_selector, timeout=TIMEOUTS["element_wait"])
        logger.info("找到验证码图片元素。")
        image_buffer = await captcha_image_element.screenshot()

        # 使用 ddddocr 识别验证码
        ocr = ddddocr.DdddOcr()
        captcha_text = ocr.classification(image_buffer)
        logger.info(f"识别到的验证码: {captcha_text}")

        if captcha_text:
            await page.type(captcha_input_selector, captcha_text, delay=random.randint(50, 150))
            logger.info("验证码已填入。")
            await page.click(captcha_submit_selector)
            logger.info("验证码提交按钮已点击。")
            await page.wait_for_load_state("networkidle") # 等待页面加载完成
        else:
            logger.warning("ddddocr 未能识别出验证码。")
            raise Exception("ddddocr 未能识别出验证码")

    except PlaywrightTimeoutError:
        logger.info("未在页面上找到验证码元素，可能没有验证码或选择器不正确。")
        raise Exception("未找到验证码元素")
    except Exception as e:
        logger.error(f"处理验证码时发生错误: {e}")
        await page.screenshot(path="captcha_error.png")
        raise Exception(f"验证码处理失败: {e}")
...
async def login(page):
    ...
    for attempt in range(max_attempts):
        try:
            ...
            logger.warning(f"尝试 {attempt + 1} 失败：在180秒内未检测到登录输入框。可能存在验证码。")
            try:
                # 尝试解决验证码
                await solve_captcha(page)
                logger.info("验证码尝试解决完成，重新等待登录表单。")
            except Exception as captcha_e:
                logger.error(f"验证码处理失败: {captcha_e}")
                if attempt == max_attempts - 1:
                    ...
                    send_bark_notification(
                        "DigitalPlat 登录失败",
                        f"多次尝试后未能跳过人机验证或验证码处理失败: {captcha_e}，请检查截图和页面源代码。",
                        level="timeSensitive"
                    )
                    raise Exception("登录失败：无法跳过人机验证或验证码处理失败")
...
async def run_renewal():
    ...
    async with async_playwright() as p:
        try:
            ...
            if not domain_rows:
                logger.info("未找到任何域名。")
                send_bark_notification("DigitalPlat 续期", "未找到任何域名进行续期。", level="passive")
                return

            logger.info(f"找到 {len(domain_rows)} 个域名，开始处理...")

            for i, row in enumerate(domain_rows):
                domain_name_element = row.locator("td:nth-child(2) a")
                domain_name = await domain_name_element.inner_text()
                domain_url_path = await domain_name_element.get_attribute("href")

                if domain_url_path:
                    logger.info(f"\n--- 正在处理域名: {domain_name} ---")
                    is_renewed, error = await process_domain(page, domain_name, domain_url_path, "https://dash.domain.digitalplat.org")
                    if is_renewed is True:
                        renewed_domains.append(domain_name)
                    elif is_renewed is False:
                        failed_domains.append(error)
                else:
                    logger.warning(f"域名 {domain_name} 未找到有效的管理链接。")

            logger.info("\n所有域名处理完毕。")

        except Exception as e:
            logger.critical(f"脚本运行过程中发生严重错误: {e}")
            send_bark_notification("DigitalPlat 脚本严重错误", f"脚本运行过程中发生严重错误: {e}", level="timeSensitive")
        finally:
            if browser:
                await browser.close()
                logger.info("浏览器已关闭。")
            save_results(renewed_domains, failed_domains)
            final_message = f"DigitalPlat 续期完成！成功续期 {len(renewed_domains)} 个，失败 {len(failed_domains)} 个。"
            if failed_domains:
                final_message += f" 失败域名: {'; '.join(failed_domains)}"
            send_bark_notification("DigitalPlat 续期结果", final_message)
